...
function sign(timestamp: string, method: string, requestPath: string, body = '') {
  ...
  if (!secret) {
    throw new Error('BITGET_API_SECRET is missing')
  }
...
async function bitgetGet(path: string, query: Record<string, string> = {}) {
  ...
  if (!apiKey || !passphrase) {
    throw new Error('Bitget environment variables are missing')
  }
...
export async function GET() {
  try {
    const accounts = await bitgetGet('/api/v2/mix/account/accounts', {
